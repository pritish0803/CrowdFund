...
it('requires a min contribute',async()=>{
try {
  await campaign.methods.contribute().send({
    value:'5',
    from:accounts[1]
...
await campaign.methods.createRequest('buy coin','100',accounts[1])
.send({
  from:accounts[0],
  gas:'1000000'
});
const requests=await campaign.methods.request(0).call();
assert.equal('buy coin',requests.description);
